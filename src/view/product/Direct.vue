<template>
  <div class="direct">
    <scroller
      ...
      <div v-if="GET_DIRCT_LIST.response">
      <div v-for="item in GET_DIRCT_LIST.response.data" @click="onItemClick(item)"
           class="row">
        {{ item.name }}
      </div>
      </div>
...
//      this.bottom = 20
    ...
    methods:{
      ...
      getData:function (){
        ...
        console.log(this.GET_DIRCT_LIST)
      },
      refresh(done) {
//        setTimeout(() => {
//          let start = this.top - 1
//          for (let i = start; i > start - 10; i--) {
//            this.items=['a','b','c','s','s','s','s','b']
//          }
